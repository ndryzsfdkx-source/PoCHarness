#!/usr/bin/env python3
# Vendored-and-modified snapshot of SEC-bench's evaluator.
# Local additions in this fork:
# - replace the shipped loose PoC oracle with four graders
# - emit one PoC report per grader from a single evaluation pass
# - add grader-focused CLI plumbing and compatibility report aliasing
"""SEC-bench evaluation module.

SEC-bench ships a single loose oracle that we consider a measurement bug; this
module replaces it with four graders at distinct epistemic thresholds —
Crash-only, Path-aware, Function-level, Source-location (see TERMINOLOGY.md).
The grading algorithm itself lives in smolagents.secb.sanitizer.parser
(vendor/smolagents/); this module is CLI/orchestration around that import.
...
try:
    from smolagents.secb.sanitizer.parser import SUPPORTED_GRADERS, grade, parse_sanitizer_output
except ImportError:
    # Fallback for the case where smolagents isn't pip-installed: walk up from
    # this file looking for a sibling vendor/smolagents/src (this fork's own
    # repo layout). Normal setup (environment.yml) installs smolagents as a
    # package, so this branch shouldn't trigger in practice.
    for parent in Path(__file__).resolve().parents:
        local_smolagents_src = parent / "vendor" / "smolagents" / "src"
        if local_smolagents_src.exists():
            sys.path.insert(0, str(local_smolagents_src))
            break
    from smolagents.secb.sanitizer.parser import SUPPORTED_GRADERS, grade, parse_sanitizer_output
...
class EvaluationResult:
    ...
    expected_sanitizer_report: Optional[str] = None
...
def run_evaluation_single(
    ...
    try:
        ...
        expected_sanitizer_report = dataset_dict[instance_id].get("sanitizer_report")
        ...
        # Create a temporary directory under a Docker-shared path.
        # On macOS, the default temp dir can be under /var/folders which may not be mountable into Docker.
        tmp_root = Path.home() / ".secb_eval_tmp"
        tmp_root.mkdir(parents=True, exist_ok=True)

        tmp_dir = Path(tempfile.mkdtemp(prefix="secb.eval.", dir=str(tmp_root)))
        try:
            if type == "patch":
                # Replace all "\r\n" with "\n" in the model_patch.
                ...
                patch_file_path = tmp_dir / "model_patch.diff"
                ...
                try:
                    ...
                    poc_file_path = tmp_dir / "poc_artifact.tar.gz"
                    ...
                    return EvaluationResult(
                        ...
                        expected_sanitizer_report=expected_sanitizer_report,
                        model_name="unknown_model",
                    )
        finally:
            # Cleanup happens after container run as well; ignore errors to avoid aborting evaluation.
            pass
        ...
        if type == "patch":
            volumes = {str(patch_file_path): {"bind": "/tmp/model_patch.diff", "mode": "ro"}}
        else:
            volumes = {str(poc_file_path): {"bind": "/tmp/poc_artifact.tar.gz", "mode": "ro"}}
        ...
        try:
            ...
            logger.warning(f"Failed to remove container for {instance_id}: {e}")
        try:
            shutil.rmtree(tmp_dir, ignore_errors=True)
        except Exception as e:
            logger.warning(f"Failed to cleanup tmp_dir for {instance_id}: {e}")
        ...
        error_msg = f"Unhandled evaluator exception for {instance_id}: {e}"
        ...
        return EvaluationResult(
            ...
            expected_sanitizer_report=dataset_dict.get(instance_id, {}).get(
                "sanitizer_report"
            ),
            model_name="unknown_model",
        )
...
def log_poc_oracle_coverage(results: list[EvaluationResult]) -> None:
    missing_ref = sum(1 for result in results if not result.expected_sanitizer_report)
    logger.info(
        f"PoC grader coverage: {len(results)} instances, "
        f"{missing_ref} missing expected_sanitizer_report"
    )


def interpret_poc_results(
    results: list[EvaluationResult],
    grader: str = "loose",
    line_tolerance: int = 10,
) -> list[PoCResult]:
    """Interpret PoC results using one named grader over parsed sanitizer reports."""
    logger.info(
        f"Interpreting PoC results with grader={grader} "
    ...
    for result in results:
        ...
        # Check if the PoC execution reached the final step
            ...
            expected_report = (
                parse_sanitizer_output(result.expected_sanitizer_report)
                if result.expected_sanitizer_report
                else None
            )
            actual_report = parse_sanitizer_output(result.sanitizer_report)
            match_result = grade(
                expected_report,
                actual_report,
                grader=grader,
            ...
            success = bool(match_result["pass"])
            if match_result["reason"] == "no_oracle":
                step_reason = (
                    "PoC triggered a sanitizer error, but the dataset instance does "
                    f"not provide an expected sanitizer report for {grader} grading."
                )
            elif success:
                step_reason = (
                    f"PoC matched the expected sanitizer report under the {grader} grader."
                )
            else:
                step_reason = (
                    f"PoC triggered a sanitizer error but failed the {grader} grader: "
...
def interpret_results(
    ...
    return interpret_poc_results(
        results,
        grader=grader or "loose",
        line_tolerance=grader_line_tolerance,
    )
...
def write_compat_alias(report_path: Path, alias_name: str) -> None:
    alias_path = report_path.parent / alias_name
    if alias_path == report_path:
        return
    if alias_path.exists() or alias_path.is_symlink():
        alias_path.unlink()
    try:
        alias_path.symlink_to(report_path.name)
        logger.info(f"Created compatibility symlink: {alias_path} -> {report_path.name}")
    except OSError:
        shutil.copy2(report_path, alias_path)
        logger.info(f"Created compatibility copy: {alias_path}")
...
def save_results(
    ...
    compat_aliases: Optional[list[str]] = None,
) -> None:
    ...
    """
    filename = filename_override or f"report_{mode}.jsonl"
    compat_aliases = compat_aliases or []
    ...
    if output_dir:
        if agent in ["swea", "oh", "smolagent"]:
            ...
            write_results_file(report_path, results)
            for alias_name in compat_aliases:
                write_compat_alias(report_path, alias_name)
        else:
            ...
            for model_name, model_results in output_results_by_model.items():
                ...
                write_results_file(report_path, model_results)
                for alias_name in compat_aliases:
                    write_compat_alias(report_path, alias_name)
        return
...
def parse_graders_arg(raw: str) -> list[str]:
    graders = [grader.strip() for grader in raw.split(",") if grader.strip()]
    if not graders:
        raise ValueError("At least one grader must be specified.")

    invalid = [grader for grader in graders if grader not in SUPPORTED_GRADERS]
    if invalid:
        raise ValueError(
            f"Unsupported graders: {', '.join(invalid)}. "
            f"Supported graders: {', '.join(SUPPORTED_GRADERS)}"
        )

    deduped: list[str] = []
    for grader in graders:
        if grader not in deduped:
            deduped.append(grader)
    return deduped


def poc_report_filename(grader: str, agent: str) -> str:
    return f"report_{grader}_{agent}.jsonl"
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            "For PoC evaluation only, comma-separated grader list. "
            "Default: loose,caller,semantic,strict"
        ),
    )
    parser.add_argument(
    ...
    try:
        ...
        if args.type == "patch":
            ...
            graders = parse_graders_arg(args.graders)
            log_poc_oracle_coverage(raw_results)
            for grader_name in graders:
                poc_results = interpret_poc_results(
                    raw_results,
                    grader=grader_name,
                    line_tolerance=args.grader_line_tolerance,
                )
                save_results(
                    poc_results,
                    input_path,
                    ...
                    args.agent,
                    output_path,
                    filename_override=poc_report_filename(grader_name, args.agent),
                    compat_aliases=["report_sanitizer.jsonl"] if grader_name == "loose" else None,
                )
    except Exception as e:
