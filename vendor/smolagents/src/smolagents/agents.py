...
class RunResult:
    ...
    Attributes:
        ...
        state: The final state of the agent after the run.
    ...
    output: Any | None
    state: Literal["success", "max_steps_error", "budget_exhausted", "pricing_unavailable"]
    steps: list[dict]
...
{str(additional_args)}."""
        ...
        if return_full_result:
            ...
            if termination_reason == "cost_budget_exhausted":
                state = "budget_exhausted"
            elif termination_reason == "pricing_unavailable":
                state = "pricing_unavailable"
            elif self.memory.steps and isinstance(getattr(self.memory.steps[-1], "error", None), AgentMaxStepsError):
    ...
    def _run_stream(
        ...
        final_answer = None
        budget_termination: AgentBudgetExceeded | None = None
        while not returned_final_answer and self.step_number <= max_steps:
            ...
            try:
                ...
                step_budget_termination = exc
                action_step.observations = exc.message
                action_step.action_output = exc.message
                ...
                try:
                    ...
                    step_budget_termination = step_budget_termination or exc
                    action_step.observations = exc.message
                    action_step.action_output = exc.message
                self.memory.steps.append(action_step)
                yield action_step
                self.step_number += 1
            if step_budget_termination is not None:
                budget_termination = step_budget_termination
                break

        if budget_termination is not None:
            self._termination_reason = budget_termination.reason
            final_answer = budget_termination.message
        elif not returned_final_answer and self.step_number == max_steps + 1:
            try:
                final_answer = self._handle_max_steps_reached(task)
                yield action_step
            except AgentBudgetExceeded as exc:
                self._termination_reason = exc.reason
    ...
    def _validate_final_answer(self, final_answer: Any):
        for check_function in self.final_answer_checks:
            try:
                passed = check_function(final_answer, self.memory, agent=self)
                if not passed:
                    raise AssertionError("Final answer check returned a falsey value.")
