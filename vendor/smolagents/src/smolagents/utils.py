...
class AgentBudgetExceeded(Exception):
    """Signal that an instance-wide model budget prevents another model call."""

    def __init__(self, *, reason: str, observed_cost: float, limit: float):
        self.reason = reason
        self.observed_cost = float(observed_cost)
        self.limit = float(limit)
        if reason == "pricing_unavailable":
            message = (
                "Instance run stopped because model-call pricing became unavailable. "
                "The current artifact will be preserved for evaluation."
            )
        else:
            message = (
                f"Instance run stopped after reaching the ${self.limit:.2f} cost budget "
                f"(observed ${self.observed_cost:.2f}). "
                "The current artifact will be preserved for evaluation."
            )
        super().__init__(message)
        self.message = message
